...
/**
 * The orientation provider that delivers the absolute orientation from the {@link Sensor#TYPE_GYROSCOPE Gyroscope} and {@link Sensor#TYPE_ROTATION_VECTOR Android Rotation Vector sensor}.
 * 
...
public class Fusion2OrientationProvider extends OrientationProvider {

	static Fusion2OrientationProvider instance;

	public static Fusion2OrientationProvider instance() {
		if (instance == null) {
			instance = new Fusion2OrientationProvider();
		}
		...
		case Sensor.TYPE_GYROSCOPE:
			...
			if (timestamp != 0) {
				...
				if (Math.abs(dotProd) < OUTLIER_THRESHOLD) {
					...
					quaternionGyroscope.slerp(quaternionRotationVector, interpolate, (float) (INDIRECT_INTERPOLATION_WEIGHT * gyroscopeRotationVelocity));
				...
				if (panicCounter > PANIC_THRESHOLD) {
					...
					if (gyroscopeRotationVelocity < 3) {
						...
						Log.d("Rotation Vector", String.format("Panic reset delayed due to ongoing motion (user is still shaking the device). Gyroscope Velocity: %.2f > 3", gyroscopeRotationVelocity));
					}
				}
			}
			break;
