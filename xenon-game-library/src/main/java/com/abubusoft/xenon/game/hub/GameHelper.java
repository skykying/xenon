...
public class GameHelper implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener {
	...
	public interface GameHelperListener {
		 ...
		 * Note that not all calls to this method mean an error; it may be a result of the fact that automatic sign-in could not proceed because user interaction was required
		 * (consent dialogs). So implementations of this method should NOT display an error message unless a call to
		 * 
	...
	/**
	 * The Activity we are bound to. We need to keep a reference to the Activity because some games methods require an Activity (a Context won't do). We are careful not to leak
	 * these references: we release them on onStop().
	 */
	...
	// sign-in flow.
	final static int RC_RESOLVE_SIGNIN = 9001;
	...
	/*
	 * Whether user has specifically requested that the sign-in process begin. If mUserInitiatedSignIn is false, we're in the automatic sign-in attempt that we try once the
	 * Activity is started -- if true, then the user has already clicked a "Sign-In" button or something similar
	 */
	...
	/*
	 * If we got an invitation when we connected to the games client, it's here. Otherwise, it's null.
	 */
	Invitation mInvitation;

	/*
	 * If we got turn-based match when we connected to the games client, it's here. Otherwise, it's null.
	 */
	TurnBasedMatch mTurnBasedMatch;

	/*
	 * If we have incoming requests when we connected to the games client, they are here. Otherwise, it's null.
	 */
	 ...
	 * 
	 * @param clientsToUse
	 *            the API clients to use (a combination of the CLIENT_* flags, or CLIENT_ALL to mean all clients).
	 */
	public GameHelper(Activity act, int clientsToUse) {
		activity = act;
	...
	/**
	 * Sets the maximum number of automatic sign-in attempts to be made on application startup. This maximum is over the lifetime of the application (it is stored in a
	 * SharedPreferences file). So, for example, if you specify 2, then it means that the user will be prompted to sign in on app startup the first time and, if they cancel, a
	 * second time the next time the app starts, and, if they cancel that one, never again. Set to 0 if you do not want the user to be prompted to sign in on application startup.
	 */
	...
	/**
	 * Creates a GoogleApiClient.Builder for use with @link{#setup}. Normally, you do not have to do this; use this method only if you need to make nonstandard setup (e.g. adding
	...
	/**
	 * Performs setup on this GameHelper object. Call this from the onCreate() method of your Activity. This will create the clients and do a few other initialization tasks. Next,
	 ...
	 * 
	 * @link{GameHelperListener#onSignInSucceeded method, to check if there's an invitation available. In that case, accept the invitation.
	 ...
	 * 
	 * @link{GameHelperListener#onSignInSucceeded method, to check if there are incoming requests that must be handled.
	...
	public void onActivityResult(int requestCode, int responseCode, Intent intent) {
		debugLog("onActivityResult: req=" + (requestCode == RC_RESOLVE_SIGNIN ? "RC_RESOLVE_SIGNIN" : String.valueOf(requestCode)) + ", resp=" + GameHelperUtils.activityResponseCodeToString(responseCode));
	...
	/**
	 * Starts a user-initiated sign-in flow. This should be called when the user clicks on a "Sign In" button. As a result, authentication/consent dialogs may show up. At the end
	 * of the process, the GameHelperListener's onSignInSucceeded() or onSignInFailed() methods will be called.
	 */
	public void beginUserInitiatedSignIn() {
		...
		if (mGoogleApiClient.isConnected()) {
			...
			logWarn("beginUserInitiatedSignIn() called when already connecting. " + "Be patient! You can only call this method after you get an " + "onSignInSucceeded() or onSignInFailed() callback. Suggestion: disable "
					+ "the sign-in button on startup and also when it's clicked, and re-enable " + "when you get the callback.");
	...
	int getSignInCancellations() {
		SharedPreferences sp = context.getSharedPreferences(GAMEHELPER_SHARED_PREFS, Context.MODE_PRIVATE);
	...
	int incrementSignInCancellations() {
		...
		SharedPreferences.Editor editor = context.getSharedPreferences(GAMEHELPER_SHARED_PREFS, Context.MODE_PRIVATE).edit();
	...
	void resolveConnectionResult() {
		...
		if (mConnectionResult.hasResolution()) {
			...
			try {
				...
				mConnectionResult.startResolutionForResult(activity, RC_RESOLVE_SIGNIN);
	...
	/**
	 * Give up on signing in due to an error. Shows the appropriate error message to the user, using a standard error dialog as appropriate to the cause of the error. That dialog
	 * will indicate to the user how the problem can be solved (for example, re-enable Google Play Services, upgrade to a new version, etc).
	 */
	void giveUp(SignInFailureReason reason) {
		...
		if (reason.mActivityResultCode == GamesActivityResultCodes.RESULT_APP_MISCONFIGURED) {
			...
			GameHelperUtils.printMisconfiguredDebugInfo(context);
		}
	...
	public Dialog makeSimpleDialog(String text) {
		...
		return makeSimpleDialog(activity, text);
	}
	...
	public Dialog makeSimpleDialog(String title, String text) {
		...
		return makeSimpleDialog(activity, title, text);
	}

	void debugLog(String message) {
		ElioLogger.debug("GameHelper: " + message);
	}

	void logWarn(String message) {
		ElioLogger.warn("!!! GameHelper WARNING: " + message);
	}

	void logError(String message) {
		ElioLogger.error(TAG, "*** GameHelper ERROR: " + message);
	}
	...
	public static class SignInFailureReason {
		...
		public String toString() {
			return "SignInFailureReason(serviceErrorCode:" + GameHelperUtils.errorCodeToString(mServiceErrorCode)
					+ ((mActivityResultCode == NO_ACTIVITY_RESULT_CODE) ? ")" : (",activityResultCode:" + GameHelperUtils.activityResponseCodeToString(mActivityResultCode) + ")"));
		}
	}
