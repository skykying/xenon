...
public class GameHub {
    ...
    public void onActivityStart(Activity activity) {
        //config = XenonBeanContext.getBean(XenonBeanType.CONFIG);
        ...
        if (config.googleAutoSignin) {
            if (helper.mGoogleApiClient.isConnected()) {
                XenonLogger.warn("GameHelper: client was already connected on onStart()");
            } else {
    ...
    public void showAlert(String message) {
        if (helper == null) {
            XenonLogger.error("showAlert, no helper found in gamehub");
            return;
        }
    ...
    public void updateAccomplishments() {
        ...
        for (int i = 0; i < achievementsManager.list.size(); i++) {
            ...
            if (item.checker == null) {
                XenonLogger.warn("Achievement %s can not be unlocked", getString(item.resourceName));
                continue;
            }
            ...
            try {
                ...
                if (item.checker.check(item, config) || alreadyDone) {
                    ...
                    if (isSignedIn()) {
                        ...
                        XenonLogger.warn("Achievement %s can not be uploaded");
                    }
                ...
                XenonLogger.warn("Achievement %s (%s) check has an error %s", getString(item.uidResourceId), getString(item.resourceName), e.getMessage());
                e.printStackTrace();
            }
        }
    ...
    public void updateScore(int leaderBoardId, int finalScore) {
        ...
        if (helper.isSignedIn()) {
            ...
            Games.Leaderboards.submitScore(helper.getApiClient(), ResourceUtility.resolveString(argon.context(), leaderBoardId), config.score);
        } else {
            XenonLogger.warn("Can not save on cloud");
        }
