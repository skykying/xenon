...
public class Vector3 extends Point3 {
	...
	public static Vector3 unit(Vector3 a, Vector3 result) {
		a.copyInto(result);
		result.normalize();
	...
	public static void fromAngles(float theta, float phi, Vector3 result) {
		result.setCoords((float) (Math.cos(theta) * Math.cos(phi)), (float) (Math.sin(phi)), (float) (Math.sin(theta) * Math.cos(phi)));
	};

	public static void randomDirection(Vector3 result) {
		fromAngles((float) (Math.random() * Math.PI * 2), (float) (Math.asin(Math.random() * 2 - 1)), result);
	};
