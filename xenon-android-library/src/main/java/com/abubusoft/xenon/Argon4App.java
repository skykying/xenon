...
public class Argon4App extends Argon4BaseImpl<ArgonApplication4App>  {
	 ...
	 * </ol>
	 * 
	 * Argon rileva se è la prima volta che viene eseguita l'applicazione
	 ...
	 * @throws Exception
	 */
	/*public void onStartup(ArgonActivity4App activity, ArgonWallpaper service) throws Exception {
		...
		if (firstTime) {
			...
			settings = (ArgonSettings) am.attributes.get(ApplicationManager.SETTINGS_KEY);

			// 4 - creiamo application (recuperato da settings)
			application = (ArgonApplication4App) Class.forName(settings.application.clazz).newInstance();
