	...
	public Point2 touch(float screenX, float screenY) {
		{
			...
			ElioLogger.info("Isometric map position (%s , %s), window size = %s, tile size = %s", map.positionInMap.x, map.positionInMap.y, ((IsometricMapHandler)(map.handler)).isoWindowSize, ((IsometricMapHandler)(map.handler)).isoTileSize);
		}
		...
		
		{
			...
			ElioLogger.info("Convert raw screen (%s , %s) to tile (%s , %s)", screenX, screenY, workScroll.x, workScroll.y);
		}
